...
function __powerline_user_info_prompt {
  ...
  HOSTNAME=$(hostname -f | sed 's/.tacc.utexas.edu//')
...
function __powerline_python_venv_prompt {
  ...
  if [[ -n "${CONDA_DEFAULT_ENV}" ]]; then
    ...
    python_venv="${python_venv##*/}"   # strip path off
...
function __powerline_cwd_prompt {
  ...
  dir_name=$(pwd | sed "s|^${HOME}|~|" | sed "s|^{WORK}|\$WORK|" | rev | cut -d/ -f1,2 | rev )
  echo "${dir_name}|${CWD_THEME_PROMPT_COLOR}"
}
